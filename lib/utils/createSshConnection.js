...
export async function createSshConnection({ ip, username, port = 22, privateKey, otpSecret, password }) {
  const config = {
    host: ip,
    port,
    username,
  ...
  const connectionPromise = new Promise((resolve, reject) => {
    conn.on('ready', () => {
      console.log(kleur.green('con: ready'), { host: config.host, port: config.port, username });
      resolve();
    })
    .on('error', (err) => {
